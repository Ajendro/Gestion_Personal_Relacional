const PersonalObjectives = require('../models/personal_objetivesModel'); // Adjust the path if needed

// Crear un objetivo personal
exports.createPersonalObjective = (req, res) => {
    const { description, start_date, end_date, progress, fk_idUser } = req.body;
    PersonalObjectives.createPersonalObjective(description, start_date, end_date, progress, fk_idUser, (err, id) => {
        if (err) return res.status(500).json({ error: err.message });
        res.status(201).json({ id, description, start_date, end_date, progress, fk_idUser });
    });
};
...
exports.updatePersonalObjective = (req, res) => {
    const { idPersonal_objetives, fk_idUser } = req.params;
    const { description, start_date, end_date, progress } = req.body;
    PersonalObjectives.updatePersonalObjective(idPersonal_objetives, fk_idUser, description, start_date, end_date, progress, (err, affectedRows) => {
        if (err) return res.status(500).json({ error: err.message });
